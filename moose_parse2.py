...
# Need to replace this with alpha, digit, alnum classes 
class CallbackParser(Rule):
    ...
    def __str__(self):
        ...
        @return: A human-readable description of this rule.
        """
        return "CallbackParser(%s)" % str(self.rule.__name__)
    ...
    def match(self, input_reader):
        ...
        input_reader.checkPoint()
        try:
            self.char = input_reader.getChar()
        except EndOfStringException:
            input_reader.rollback()
            #raise ParseException('Preliminary end of string')
        if self.rule(self.char):
            input_reader.deleteCheckpoint()
            return self.returnToken(self.callAction([self.char]))
        else:
            input_reader.rollback()
            pass 
...
alnum    = lambda char: char.isalnum()    
integer  = Word(digit) 
